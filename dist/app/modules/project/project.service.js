...
const createSalesIntoDB = (Data) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const productData = yield blog_model_1.Blog.findById(productId);
    if (!productData) {
    ...
    const sale = new project_model_1.Sale({
        productId,
        quantity,
        buyerName,
        saleDate,
    ...
    if (productData.quantity === 0) {
        yield blog_model_1.Blog.findByIdAndDelete(productId);
    }
    else {
...
const getSaleHistoryFromDB = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // );
    const weeklyData = yield project_model_1.Sale.aggregate([
        {
            $group: {
                _id: { $week: '$saleDate' },
    ...
    const dailyData = yield project_model_1.Sale.aggregate([
        {
            $group: {
    ...
    const monthlyData = yield project_model_1.Sale.aggregate([
        {
            $group: {
                _id: { $month: '$saleDate' },
                totalSales: { $sum: '$quantity' },
    ...
    const yearlyData = yield project_model_1.Sale.aggregate([
        {
            $group: {
                _id: { $year: '$saleDate' },
                totalSales: { $sum: '$quantity' },
